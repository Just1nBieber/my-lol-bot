...
## ✨ Feature (新增特性)
- **优化**: 采用 `shallowRef` 与 `markRaw` 结合 `localForage` 实现了对 **静态资源数据和 10 人对局详情庞大数据的按需独立缓存**。彻底物理隔绝了 Vue 深度代理（Proxy）带来的内存开销，解决了巨型 JSON 渲染时的 UI 卡顿问题。
- **新增**: 底层通讯架构新增 `get-match-detail` IPC 数据通道，配合前端实现了战绩详情的**懒加载功能**。数据按需点对点索取，完美规避了全局状态（Render-sync）广播全量数据导致的性能风暴。